...
void rebalanceTree(Tree* t, TNode* x) {
    ...
    if (x != NULL) {
        if (x->pParent != NULL) {
            if (getBalance(x) > 1) {
                if ((getBalance(x) + getBalance(x->pLeft)) > getBalance(x)) {
                    rightRotate(t, x);
                    rebalanceTree(t, x->pParent);
                }
                else {
                ...
                if ((getBalance(x->pRight) + getBalance(x)) < getBalance(x)) {
                    leftRotate(t, x);
                    rebalanceTree(t, x->pParent);
                }
                else {
...
int lineStabQuery(TNode* root, double queryPoint) {
    ...
    if (queryPoint >= root->low && queryPoint <= root->high) {
        count += root->cnt;
        if (root->pLeft != NULL && (queryPoint >= root->pLeft->low && queryPoint <= root->pLeft->high)) {
            count = lineStabQuery(root->pLeft, queryPoint);
        }
        if (root->pRight != NULL && (queryPoint >= root->pRight->low && queryPoint <= root->pRight->high)) {
